//source: https://lemire.me/blog/2022/12/23/fast-base16-encoding/ and https://github.com/lemire/Code-used-on-Daniel-Lemire-s-blog/tree/master/2022/12/23
...
void encode_byte_table(const uint8_t *source, size_t len, char *target) {
...
// This version is inspired by the hand vectorized code for x86.
void encode_nibble_table(const uint8_t *source, size_t len, char *target) {
  const uint16_t table[] = { 'f', 'e', 'd', 'c',
                             'b', 'a', '9', '8', 
                             '7', '6', '5', '4',
                             '3', '2', '1', '0'};
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    uint8_t nib2 = b >> 4;
    if (0) {
      // This simpler version hits a known deficency around
      // interleave stores in the vectorizer right now.
      {
        uint16_t code = table[nib1];
        memcpy(target, &code, 2);
        target += 2;
      }
      {
        uint16_t code = table[nib2];
        memcpy(target, &code, 2);
        target += 2;
      }
    } else {
      // Hand rolled, probably not endian correct
      uint16_t code1 = table[nib1];
      uint16_t code2 = table[nib2];
      uint32_t temp = code1 | ((uint32_t)code2) << 16;
      memcpy(target, &temp, 4);
      target += 4;
    }
  }
}
