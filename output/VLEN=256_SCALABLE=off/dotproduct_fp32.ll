...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi float [ 0.000000e+00, %vector.ph ], [ %7, %vector.body ]
  ...
  %wide.load10 = load <8 x float>, ptr %1, align 4, !tbaa !4
  %2 = getelementptr inbounds float, ptr %b, i64 %index
  %wide.load11 = load <8 x float>, ptr %2, align 4, !tbaa !4
  %3 = getelementptr inbounds float, ptr %2, i64 8
  %wide.load12 = load <8 x float>, ptr %3, align 4, !tbaa !4
  %4 = fmul <8 x float> %wide.load, %wide.load11
  %5 = fmul <8 x float> %wide.load10, %wide.load12
  %6 = tail call float @llvm.vector.reduce.fadd.v8f32(float %vec.phi, <8 x float> %4)
  %7 = tail call float @llvm.vector.reduce.fadd.v8f32(float %6, <8 x float> %5)
...
for.body.preheader13:                             ; preds = %for.body.preheader, %middle.block
  ...
  %res.08.ph = phi float [ 0.000000e+00, %for.body.preheader ], [ %7, %middle.block ]
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body, %middle.block, %entry
  %res.0.lcssa = phi float [ 0.000000e+00, %entry ], [ %7, %middle.block ], [ %11, %for.body ]
...
for.body:                                         ; preds = %for.body.preheader13, %for.body
  ...
  %res.08 = phi float [ %11, %for.body ], [ %res.08.ph, %for.body.preheader13 ]
  %arrayidx = getelementptr inbounds float, ptr %a, i64 %indvars.iv
  %9 = load float, ptr %arrayidx, align 4, !tbaa !4
  %arrayidx2 = getelementptr inbounds float, ptr %b, i64 %indvars.iv
  %10 = load float, ptr %arrayidx2, align 4, !tbaa !4
  %11 = tail call float @llvm.fmuladd.f32(float %9, float %10, float %res.08)
