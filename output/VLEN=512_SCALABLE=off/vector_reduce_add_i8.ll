...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <16 x i32> [ zeroinitializer, %vector.ph ], [ %4, %vector.body ]
  %vec.phi8 = phi <16 x i32> [ zeroinitializer, %vector.ph ], [ %5, %vector.body ]
  %0 = getelementptr inbounds i8, ptr %a, i64 %index
  %wide.load = load <16 x i8>, ptr %0, align 1, !tbaa !4
  %1 = getelementptr inbounds i8, ptr %0, i64 16
  %wide.load9 = load <16 x i8>, ptr %1, align 1, !tbaa !4
  %2 = zext <16 x i8> %wide.load to <16 x i32>
  %3 = zext <16 x i8> %wide.load9 to <16 x i32>
  %4 = add <16 x i32> %vec.phi, %2
  %5 = add <16 x i32> %vec.phi8, %3
  %index.next = add nuw i64 %index, 32
  %6 = icmp eq i64 %index.next, %n.vec
  br i1 %6, label %middle.block, label %vector.body, !llvm.loop !7

middle.block:                                     ; preds = %vector.body
  %bin.rdx = add <16 x i32> %5, %4
  %7 = tail call i32 @llvm.vector.reduce.add.v16i32(<16 x i32> %bin.rdx)
...
vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  ...
  %vec.phi14 = phi <8 x i32> [ %8, %vec.epilog.ph ], [ %11, %vec.epilog.vector.body ]
  %9 = getelementptr inbounds i8, ptr %a, i64 %offset.idx
  %wide.load15 = load <8 x i8>, ptr %9, align 1, !tbaa !4
  %10 = zext <8 x i8> %wide.load15 to <8 x i32>
  %11 = add <8 x i32> %vec.phi14, %10
...
vec.epilog.middle.block:                          ; preds = %vec.epilog.vector.body
  %13 = tail call i32 @llvm.vector.reduce.add.v8i32(<8 x i32> %11)
