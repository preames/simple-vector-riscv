...
vector.ph:                                        ; preds = %vector.memcheck
  %n.vec = and i64 %n, -32
  %broadcast.splatinsert = insertelement <16 x float> poison, float %a, i64 0
  %broadcast.splat = shufflevector <16 x float> %broadcast.splatinsert, <16 x float> poison, <16 x i32> zeroinitializer
  %broadcast.splatinsert14 = insertelement <16 x float> poison, float %a, i64 0
  %broadcast.splat15 = shufflevector <16 x float> %broadcast.splatinsert14, <16 x float> poison, <16 x i32> zeroinitializer
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ]
  %1 = getelementptr inbounds float, ptr %x, i64 %index
  %wide.load = load <16 x float>, ptr %1, align 4, !tbaa !4, !alias.scope !8
  %2 = getelementptr inbounds float, ptr %1, i64 16
  %wide.load11 = load <16 x float>, ptr %2, align 4, !tbaa !4, !alias.scope !8
  %3 = getelementptr inbounds float, ptr %y, i64 %index
  %wide.load12 = load <16 x float>, ptr %3, align 4, !tbaa !4, !alias.scope !11, !noalias !8
  %4 = getelementptr inbounds float, ptr %3, i64 16
  %wide.load13 = load <16 x float>, ptr %4, align 4, !tbaa !4, !alias.scope !11, !noalias !8
  %5 = tail call <16 x float> @llvm.fmuladd.v16f32(<16 x float> %broadcast.splat, <16 x float> %wide.load, <16 x float> %wide.load12)
  %6 = tail call <16 x float> @llvm.fmuladd.v16f32(<16 x float> %broadcast.splat15, <16 x float> %wide.load11, <16 x float> %wide.load13)
