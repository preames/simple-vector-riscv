...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <vscale x 4 x i32> [ zeroinitializer, %vector.ph ], [ %6, %vector.body ]
...
middle.block:                                     ; preds = %vector.body
  %8 = tail call i32 @llvm.vector.reduce.add.nxv4i32(<vscale x 4 x i32> %6)
...
for.body.preheader7:                              ; preds = %for.body.preheader, %middle.block
  ...
  %sum.05.ph = phi i32 [ 0, %for.body.preheader ], [ %8, %middle.block ]
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body, %middle.block, %entry
  %sum.0.lcssa = phi i32 [ 0, %entry ], [ %8, %middle.block ], [ %add, %for.body ]
  ret i32 %sum.0.lcssa
