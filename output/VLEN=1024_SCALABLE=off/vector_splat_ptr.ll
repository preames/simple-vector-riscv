...
vector.main.loop.iter.check:                      ; preds = %iter.check
  %min.iters.check5 = icmp ult i32 %a_len, 32
  br i1 %min.iters.check5, label %vec.epilog.ph, label %vector.ph

vector.ph:                                        ; preds = %vector.main.loop.iter.check
  %n.vec = and i64 %wide.trip.count, 4294967264
  %broadcast.splatinsert = insertelement <16 x ptr> poison, ptr %v, i64 0
  %broadcast.splat = shufflevector <16 x ptr> %broadcast.splatinsert, <16 x ptr> poison, <16 x i32> zeroinitializer
  %broadcast.splatinsert6 = insertelement <16 x ptr> poison, ptr %v, i64 0
  %broadcast.splat7 = shufflevector <16 x ptr> %broadcast.splatinsert6, <16 x ptr> poison, <16 x i32> zeroinitializer
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ]
  %0 = getelementptr inbounds ptr, ptr %a, i64 %index
  store <16 x ptr> %broadcast.splat, ptr %0, align 8, !tbaa !4
  %1 = getelementptr inbounds ptr, ptr %0, i64 16
  store <16 x ptr> %broadcast.splat7, ptr %1, align 8, !tbaa !4
...
vec.epilog.ph:                                    ; preds = %vector.main.loop.iter.check, %vec.epilog.iter.check
  ...
  %n.vec9 = and i64 %wide.trip.count, 4294967288
  %broadcast.splatinsert12 = insertelement <8 x ptr> poison, ptr %v, i64 0
  %broadcast.splat13 = shufflevector <8 x ptr> %broadcast.splatinsert12, <8 x ptr> poison, <8 x i32> zeroinitializer
  br label %vec.epilog.vector.body

vec.epilog.vector.body:                           ; preds = %vec.epilog.vector.body, %vec.epilog.ph
  %offset.idx = phi i64 [ %vec.epilog.resume.val, %vec.epilog.ph ], [ %index.next14, %vec.epilog.vector.body ]
  %3 = getelementptr inbounds ptr, ptr %a, i64 %offset.idx
  store <8 x ptr> %broadcast.splat13, ptr %3, align 8, !tbaa !4
  %index.next14 = add nuw i64 %offset.idx, 8
