...
vector.ph:                                        ; preds = %for.body.preheader
  ...
  %vector.recur.init = insertelement <4 x i32> poison, i32 %.pre19, i64 3
  %vector.recur.init20 = insertelement <4 x i32> poison, i32 %.pre, i64 3
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ]
  %vector.recur = phi <4 x i32> [ %vector.recur.init, %vector.ph ], [ %wide.load, %vector.body ]
  %vector.recur21 = phi <4 x i32> [ %vector.recur.init20, %vector.ph ], [ %3, %vector.body ]
  ...
  %4 = shufflevector <4 x i32> %vector.recur21, <4 x i32> %3, <4 x i32> <i32 3, i32 4, i32 5, i32 6>
  %5 = add nsw <4 x i32> %3, %4
  %6 = add nsw <4 x i32> %5, %wide.load
  %7 = getelementptr inbounds i32, ptr %b, i64 %offset.idx
  store <4 x i32> %6, ptr %7, align 4, !tbaa !4
...
middle.block:                                     ; preds = %vector.body
  ...
  %vector.recur.extract22 = extractelement <4 x i32> %wide.load, i64 2
