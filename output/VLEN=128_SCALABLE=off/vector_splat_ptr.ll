...
vector.ph:                                        ; preds = %for.body.preheader
  ...
  %broadcast.splatinsert = insertelement <2 x ptr> poison, ptr %v, i64 0
  %broadcast.splat = shufflevector <2 x ptr> %broadcast.splatinsert, <2 x ptr> poison, <2 x i32> zeroinitializer
  %broadcast.splatinsert5 = insertelement <2 x ptr> poison, ptr %v, i64 0
  %broadcast.splat6 = shufflevector <2 x ptr> %broadcast.splatinsert5, <2 x ptr> poison, <2 x i32> zeroinitializer
  br label %vector.body

vector.body:                                      ; preds = %vector.body, %vector.ph
  %index = phi i64 [ 0, %vector.ph ], [ %index.next, %vector.body ]
  %0 = getelementptr inbounds ptr, ptr %a, i64 %index
  store <2 x ptr> %broadcast.splat, ptr %0, align 8, !tbaa !4
  %1 = getelementptr inbounds ptr, ptr %0, i64 2
  store <2 x ptr> %broadcast.splat6, ptr %1, align 8, !tbaa !4
...
attributes #0 = { nofree norecurse nosync nounwind memory(argmem: write) vscale_range(2,1024) "frame-pointer"="none" "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="generic-rv64" "target-features"="+64bit,+a,+c,+d,+f,+m,+relax,+v,+zba,+zbb,+zbc,+zbs,+zve32f,+zve32x,+zve64d,+zve64f,+zve64x,+zvl128b,+zvl32b,+zvl64b,-save-restore" }
