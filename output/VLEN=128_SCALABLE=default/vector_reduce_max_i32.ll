...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi <vscale x 4 x i32> [ shufflevector (<vscale x 4 x i32> insertelement (<vscale x 4 x i32> poison, i32 -987654321, i64 0), <vscale x 4 x i32> poison, <vscale x 4 x i32> zeroinitializer), %vector.ph ], [ %7, %vector.body ]
  %6 = getelementptr inbounds i32, ptr %a, i64 %index
  %wide.load = load <vscale x 4 x i32>, ptr %6, align 4, !tbaa !4
  %7 = tail call <vscale x 4 x i32> @llvm.smax.nxv4i32(<vscale x 4 x i32> %wide.load, <vscale x 4 x i32> %vec.phi)
  %index.next = add nuw i64 %index, %5
  %8 = icmp eq i64 %index.next, %n.vec
  br i1 %8, label %middle.block, label %vector.body, !llvm.loop !8

middle.block:                                     ; preds = %vector.body
  %9 = tail call i32 @llvm.vector.reduce.smax.nxv4i32(<vscale x 4 x i32> %7)
...
for.body.preheader13:                             ; preds = %for.body.preheader, %middle.block
  ...
  %max.011.ph = phi i32 [ -987654321, %for.body.preheader ], [ %9, %middle.block ]
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body, %middle.block, %entry
  %max.0.lcssa = phi i32 [ -987654321, %entry ], [ %9, %middle.block ], [ %.max.0, %for.body ]
  ret i32 %max.0.lcssa
...
for.body:                                         ; preds = %for.body.preheader13, %for.body
  ...
  %.max.0 = tail call i32 @llvm.smax.i32(i32 %10, i32 %max.011)
