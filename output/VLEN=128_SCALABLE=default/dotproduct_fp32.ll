...
vector.body:                                      ; preds = %vector.body, %vector.ph
  ...
  %vec.phi = phi float [ 0.000000e+00, %vector.ph ], [ %8, %vector.body ]
  %5 = getelementptr inbounds float, ptr %a, i64 %index
  %wide.load = load <vscale x 4 x float>, ptr %5, align 4, !tbaa !4
  %6 = getelementptr inbounds float, ptr %b, i64 %index
  %wide.load10 = load <vscale x 4 x float>, ptr %6, align 4, !tbaa !4
  %7 = fmul <vscale x 4 x float> %wide.load, %wide.load10
  %8 = tail call float @llvm.vector.reduce.fadd.nxv4f32(float %vec.phi, <vscale x 4 x float> %7)
...
for.body.preheader11:                             ; preds = %for.body.preheader, %middle.block
  ...
  %res.08.ph = phi float [ 0.000000e+00, %for.body.preheader ], [ %8, %middle.block ]
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body, %middle.block, %entry
  %res.0.lcssa = phi float [ 0.000000e+00, %entry ], [ %8, %middle.block ], [ %12, %for.body ]
...
for.body:                                         ; preds = %for.body.preheader11, %for.body
  ...
  %res.08 = phi float [ %12, %for.body ], [ %res.08.ph, %for.body.preheader11 ]
  %arrayidx = getelementptr inbounds float, ptr %a, i64 %indvars.iv
  %10 = load float, ptr %arrayidx, align 4, !tbaa !4
  %arrayidx2 = getelementptr inbounds float, ptr %b, i64 %indvars.iv
  %11 = load float, ptr %arrayidx2, align 4, !tbaa !4
  %12 = tail call float @llvm.fmuladd.f32(float %10, float %11, float %res.08)
